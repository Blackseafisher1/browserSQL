...
**Goal:** understand where data lives before writing SQL.`,question:{prompt:"What stores data in rows and columns?",options:["Index","Query","Table","View"],answer:2,explanation:"A table is the structure that holds rows and columns."},seed:e},{id:"02-nosql",module:1,title:"SQL vs NoSQL",type:"theory",file:"02-nosql.md",markdown:`# SQL vs NoSQL
...
`,markdown:"# Creating tables\n\nUse `CREATE TABLE` to define a new table. Specify column names, data types, and constraints.\n\nBasic syntax:\n\n```sql\nCREATE TABLE tablename (\n  column1 TYPE CONSTRAINTS,\n  column2 TYPE CONSTRAINTS\n);\n```\n\n**Goal:** Create a table called `people` with columns `id` (INTEGER PRIMARY KEY), `name` (TEXT NOT NULL), and `age` (INTEGER NOT NULL).",seed:"",check:{type:"schema",table:"people",columns:["id","name","age"]}},{id:"05-types",module:1,title:"Data Types Deep Dive",type:"theory",file:"05-types.md",markdown:`# Data types
...
**Goal:** Write a query that returns 100 divided by 4.`,check:{type:"result",expectedSql:"SELECT 100 / 4;"}}];var p=[{id:"06-primary",module:2,title:"Primary Keys",type:"practice",file:"06-primary.sql",sql:`CREATE TABLE projects (
  id INTEGER PRIMARY KEY AUTOINCREMENT,
  name TEXT NOT NULL
);
INSERT INTO projects (name) VALUES ('Apollo');
`,markdown:"# Primary keys\n\nA `PRIMARY KEY` uniquely identifies each row. Use `INTEGER PRIMARY KEY AUTOINCREMENT` to have SQLite automatically assign increasing IDs.\n\n```sql\nCREATE TABLE tablename (\n  id INTEGER PRIMARY KEY AUTOINCREMENT,\n  column TYPE\n);\n```\n\n**Goal:** Create a table called `projects` with an auto-incrementing `id` primary key and a `name` column (TEXT NOT NULL). Then insert a project row.",seed:"",check:{type:"pk",table:"projects",column:"id"}},{id:"07-autoincrement",order:2,module:2,title:"AUTOINCREMENT",type:"theory",file:"07-autoincrement.md",markdown:`# AUTOINCREMENT
...
**Goal:** know when AUTOINCREMENT matters.`,question:{prompt:"Without AUTOINCREMENT, what ID does a new row get after deleting the last row (ID 5)?",options:["5","6","1","Random"],answer:0,explanation:"Without AUTOINCREMENT, SQLite reuses the highest row ID (max(rowid)+1 = 5). With AUTOINCREMENT, it would be 6."},seed:""},{id:"08-foreign",order:3,module:2,title:"Foreign Keys",type:"practice",file:"07-foreign.sql",sql:`PRAGMA foreign_keys = ON;
...
**Goal:** know the difference.`,question:{prompt:"How many PRIMARY KEYs can a table have?",options:["Unlimited","Two","One","Depends on columns"],answer:2,explanation:"A table can have only one PRIMARY KEY, but multiple UNIQUE constraints."},seed:""}];var h=[{id:"12-attach",module:3,title:"Your First Database",type:"practice",file:"12-attach.sql",sql:`SELECT name FROM sqlite_master WHERE type = 'table';
