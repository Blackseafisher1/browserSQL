...
`,check:{type:"result",expectedSql:"SELECT 100 / 4;"}}];var T=[{id:"06-primary",module:3,title:"PRIMARY KEY",type:"theory",file:"06-primary.md",markdown:`# PRIMARY KEY
...
`}];var y=[{id:"25-inner-join",module:6,title:"INNER JOIN",type:"practice",file:"25-inner-join.sql",markdown:`# INNER JOIN
...
**Goal:** Show each customer name alongside their order item. Join on \`customers.id = orders.customer_id\`.`,sql:`SELECT customers.name, orders.item FROM customers INNER JOIN orders ON customers.id = orders.customer_id;
...
**Goal:** Show ALL customers and their order items. Customers without orders should still appear (item shows NULL).`,sql:`SELECT customers.name, orders.item FROM customers LEFT JOIN orders ON customers.id = orders.customer_id;
...
**Goal:** Show each employee name alongside their manager's name. Use LEFT JOIN so top-level employees (no manager) still appear.`,sql:`SELECT e.name AS employee, m.name AS manager FROM employees e LEFT JOIN employees m ON e.manager_id = m.id;
...
**Goal:** Show customer name, product name, and order quantity by joining \`customers\`, \`orders\`, and \`products\`.`,sql:`SELECT customers.name, products.name, orders.quantity FROM customers INNER JOIN orders ON customers.id = orders.customer_id INNER JOIN products ON orders.product_id = products.id;
`,seed:c,check:{type:"result",expectedSql:"SELECT customers.name, products.name, orders.quantity FROM customers INNER JOIN orders ON customers.id = orders.customer_id INNER JOIN products ON orders.product_id = products.id;"}},{id:"31-join-mastery",module:6,title:"Join Mastery",type:"practice",file:"31-join-mastery.sql",markdown:`# Join Mastery
...
**Goal:** Return names of customers who placed orders worth more than 100. Use a subquery with \`WHERE id IN\`.`,sql:`SELECT name FROM customers WHERE id IN (SELECT customer_id FROM orders WHERE price > 100);
...
**Goal:** Show each customer name alongside the number of orders they placed.`,sql:`SELECT name, (SELECT COUNT(*) FROM orders WHERE customer_id = customers.id) AS order_count FROM customers;
...
**Goal:** Find all expensive items (price > 50) by querying from a subquery that selects all orders.`,sql:`SELECT item, price FROM (SELECT * FROM orders) AS expensive WHERE price > 50;
...
**Goal:** Return items from \`orders\` that cost more than the average price.`,sql:`SELECT item, price FROM orders WHERE price > (SELECT AVG(price) FROM orders);
...
**Goal:** Return names of customers who have placed at least one order. Use EXISTS.`,sql:`SELECT name FROM customers WHERE EXISTS (SELECT 1 FROM orders WHERE customer_id = customers.id);
...
**Goal:** Use a CTE called \`avg_price\` to calculate the average price, then find all items above that average.`,sql:`WITH avg_price AS (SELECT AVG(price) AS avg FROM orders) SELECT item, price FROM orders, avg_price WHERE price > avg_price.avg;
...
**Goal:** know when to denormalize.`,question:{prompt:"When is denormalization useful?",options:["Always","When data must be unique","Never","When read performance matters more than write efficiency"],answer:3,explanation:"Denormalization speeds up reads by reducing joins, at the cost of redundant data."},seed:i}];var A=[{id:"43-what-index",module:9,title:"What is an Index?",type:"theory",file:"43-what-index.md",markdown:`# What is an Index?
...
**Goal:** know when indexes hurt more than help.`,question:{prompt:"Which column is a BAD candidate for an index?",options:["A primary key","A column with many unique values","A column with only two possible values","A foreign key"],answer:2,explanation:"Low-selectivity columns (few unique values) make poor indexes."},seed:o}];var I=[{id:"48-acid",module:10,title:"ACID Properties",type:"theory",file:"48-acid.md",markdown:`# ACID Properties
