...
**Goal:** Return \`customers.name\`, \`products.name\`, and \`orders.quantity\` by joining all three tables.`,sql:`SELECT customers.name, products.name, orders.quantity FROM customers INNER JOIN orders ON customers.id = orders.customer_id INNER JOIN products ON orders.product_id = products.id;
