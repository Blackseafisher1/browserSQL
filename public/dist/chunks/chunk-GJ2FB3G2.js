...
INSERT INTO events VALUES
  ...
  (5, 'Review', '2025-03-01');
`;var p=[{id:"01-intro",module:2,title:"What is a Database?",type:"theory",file:"01-intro.md",markdown:`# What is a Database?
...
**Goal:** know which keyword creates tables in SQL.`,question:{prompt:"Which SQL keyword creates a table?",options:["CREATE TABLE","ALTER","INSERT","UPDATE"],answer:0,explanation:"CREATE TABLE defines a new table structure."},seed:e},{id:"03-comments",module:2,title:"Comments in SQL",type:"theory",file:"03-comments.md",markdown:`# Comments in SQL
...
**Goal:** Write a query that returns 100 divided by 4.`,check:{type:"result",expectedSql:"SELECT 100 / 4;"}}];var h=[{id:"06-primary",module:3,title:"PRIMARY KEY",type:"theory",file:"06-primary.md",markdown:`# PRIMARY KEY
...
**Goal:** Create \`authors\` (id, name) and \`books\` (id, title, author_id FK).`,seed:n,check:{type:"fk",table:"books",column:"author_id"},hint:"CREATE TABLE authors (id INTEGER PRIMARY KEY, name TEXT); CREATE TABLE books (id INTEGER PRIMARY KEY, title TEXT, author_id INTEGER REFERENCES authors(id));"},{id:"11-relationship-1-1",module:3,title:"One-to-One Relationships",type:"practice",file:"11-relationship-1-1.sql",markdown:`# One-to-One Relationships
...
**Goal:** Create \`citizens\` (id, name) and \`passports\` (id PK referencing citizens, number TEXT).`,seed:"",check:{type:"fk",table:"passports",column:"id"},hint:"CREATE TABLE citizens (id INTEGER PRIMARY KEY, name TEXT); CREATE TABLE passports (id INTEGER PRIMARY KEY, number TEXT, FOREIGN KEY (id) REFERENCES citizens(id));"},{id:"09-cascade",module:3,title:"ON DELETE CASCADE",type:"theory",file:"09-cascade.md",markdown:`# ON DELETE CASCADE
...
- [ ] Add \`quantity\` column to order_items`,seed:n,check:{type:"constraints",table:"order_items",tokens:["primary key"]},hint:"CREATE TABLE orders (id INTEGER PRIMARY KEY, date TEXT); CREATE TABLE products (id INTEGER PRIMARY KEY, name TEXT); CREATE TABLE order_items (order_id INTEGER, product_id INTEGER, quantity INTEGER, PRIMARY KEY (order_id, product_id));",checklist:["Table orders with id and date","Table products with id and name","Table order_items with order_id, product_id, quantity","Composite PRIMARY KEY on (order_id, product_id)"]},{id:"14-relationship-n-m",module:3,title:"Many-to-Many Relationships",type:"practice",file:"14-relationship-n-m.sql",markdown:"# Many-to-Many Relationships\n\nN:M requires a junction table. The junction table has a composite PK to prevent duplicates.\n\nCashPal's content system: an actor can be in many movies, a movie has many actors.\n\n**Goal:** Create `actors`, `movies`, and a junction table `cast` with composite PK on (actor_id, movie_id).",seed:n,check:{type:"constraints",table:"cast",tokens:["primary key"]},hint:"CREATE TABLE actors (id INTEGER PRIMARY KEY, name TEXT); CREATE TABLE movies (id INTEGER PRIMARY KEY, title TEXT); CREATE TABLE cast (actor_id INTEGER, movie_id INTEGER, PRIMARY KEY (actor_id, movie_id), FOREIGN KEY (actor_id) REFERENCES actors(id), FOREIGN KEY (movie_id) REFERENCES movies(id));"},{id:"09-alter-table",module:3,title:"ALTER TABLE",type:"practice",file:"09-alter-table.sql",markdown:"# ALTER TABLE\n\nCashPal's `users` table needs a phone number field. You can modify existing tables with `ALTER TABLE`:\n\n```sql\nALTER TABLE table ADD COLUMN column TYPE;\nALTER TABLE table RENAME COLUMN old TO new;\nALTER TABLE table DROP COLUMN column;\n```\n\nThe `users` table has: id, name, city, age, email.\n\n**Your task:**\n- [ ] Add column `phone` (TEXT) to `users`\n- [ ] Rename `phone` to `phone_number`",seed:e,check:{type:"schema",table:"users",columns:["id","name","city","age","email","phone_number"]},hint:"ALTER TABLE users ADD COLUMN phone TEXT; then ALTER TABLE users RENAME COLUMN phone TO phone_number;",checklist:["Add column phone (TEXT) to users","Rename phone to phone_number"]},{id:"15-drop-table",module:3,title:"DROP TABLE",type:"practice",file:"11-drop-table.sql",markdown:"# DROP TABLE\n\n`DROP TABLE` removes a table and all its data permanently. Use with caution.\n\n```sql\nDROP TABLE table_name;\nDROP TABLE IF EXISTS table_name;  -- safe version\n```\n\n**Goal:** Drop the `users` table. Then run `SELECT * FROM sqlite_master WHERE type='table'` to verify it's gone.",seed:e,check:{type:"success"},hint:"DROP TABLE users;"},{id:"10-schema",module:3,title:"Schema Design",type:"theory",file:"10-schema.md",markdown:`# Schema Design
...
**Goal:** Insert 3 new users at once into \`users\`.`,seed:e,check:{type:"changes",min:3},hint:"INSERT INTO users (name, city, age) VALUES ('A', 'B', 20), ('C', 'D', 30), ('E', 'F', 40);"}];var y=[{id:"19-sort",module:5,title:"Sorting",type:"practice",file:"19-sort.sql",markdown:"# Sorting\n\nCashPal needs a directory sorted by age.\n\nUse `ORDER BY` to sort results:\n\n```sql\nSELECT columns FROM table ORDER BY column;\n```\n\nUse `ASC` (ascending, default) or `DESC` (descending).\n\n**Goal:** Return `name` and `age` from `users`, sorted by age youngest first.",seed:e,check:{type:"result",expectedSql:"SELECT name, age FROM users ORDER BY age;"}},{id:"20-limit",module:5,title:"Limiting Results",type:"practice",file:"20-limit.sql",markdown:`# Limiting Results
...
**Goal:** Return the names of the 3 oldest users, sorted oldest first.`,seed:e,check:{type:"result",expectedSql:"SELECT name FROM users ORDER BY age DESC LIMIT 3;"}},{id:"21-aggregates",module:5,title:"Aggregate Functions",type:"practice",file:"21-aggregates.sql",markdown:`# Aggregate Functions
...
**Goal:** Return the total number of users and their average age rounded to 2 decimals.`,seed:l,check:{type:"result",expectedSql:"SELECT COUNT(*), ROUND(AVG(age), 2) FROM users;"},hint:"SELECT COUNT(*), ROUND(AVG(age), 2) FROM users;"},{id:"22-group",module:5,title:"Grouping",type:"practice",file:"22-group.sql",markdown:`# Grouping
...
- [ ] Sort alphabetically`,seed:l,check:{type:"result",expectedSql:"SELECT city, COUNT(*) FROM users GROUP BY city HAVING COUNT(*) >= 2 ORDER BY city;"},hint:"GROUP BY city, then HAVING COUNT(*) >= 2",checklist:["Count users per city","Filter cities with at least 2 users","Sort alphabetically"]},{id:"23-distinct",module:5,title:"Distinct Values",type:"practice",file:"23-distinct.sql",markdown:"# Distinct Values\n\nCashPal wants a list of unique cities \u2014 no duplicates.\n\n`DISTINCT` removes duplicate values:\n\n```sql\nSELECT DISTINCT column FROM table;\n```\n\n**Goal:** Return all unique cities from `users`.",seed:e,check:{type:"result",expectedSql:"SELECT DISTINCT city FROM users;"}},{id:"24-alias",module:5,title:"Aliases",type:"practice",file:"24-alias.sql",markdown:"# Aliases\n\nCashPal's API needs specific column names in the response.\n\n`AS` renames columns in results:\n\n```sql\nSELECT column AS alias_name FROM table;\n```\n\n**Goal:** Return `name` as `user_name` and `age` as `user_age` from `users`.",seed:e,check:{type:"result",expectedSql:"SELECT name AS user_name, age AS user_age FROM users;"}},{id:"25-query-mastery",module:5,title:"Query Mastery",type:"practice",file:"25-query-mastery.sql",markdown:`# Query Mastery
...
`+e,check:{type:"result",expectedSql:"SELECT city FROM customers UNION SELECT city FROM users ORDER BY city;"},hint:"SELECT city FROM customers UNION SELECT city FROM users ORDER BY city;"}];var R=[{id:"25-inner-join",module:6,title:"INNER JOIN",type:"practice",file:"25-inner-join.sql",markdown:`# INNER JOIN
...
**Goal:** Show each employee name alongside their manager's name. Use LEFT JOIN so top-level employees (no manager) still appear.`,seed:d,check:{type:"result",expectedSql:"SELECT e.name AS employee, m.name AS manager FROM employees e LEFT JOIN employees m ON e.manager_id = m.id;"}},{id:"30-multi-join",module:6,title:"Joining Multiple Tables",type:"practice",file:"30-multi-join.sql",markdown:`# Joining Multiple Tables
...
**Goal:** Show customer name, product name, and order quantity by joining \`customers\`, \`orders\`, and \`products\`.`,seed:m,check:{type:"result",expectedSql:"SELECT customers.name, products.name, orders.quantity FROM customers INNER JOIN orders ON customers.id = orders.customer_id INNER JOIN products ON orders.product_id = products.id;"}},{id:"31-join-mastery",module:6,title:"Join Mastery",type:"practice",file:"31-join-mastery.sql",markdown:`# Join Mastery
...
- [ ] Sort by total quantity descending`,seed:m,check:{type:"result",expectedSql:"SELECT customers.name, SUM(orders.quantity) AS total_qty, COUNT(DISTINCT orders.product_id) AS distinct_products FROM customers INNER JOIN orders ON customers.id = orders.customer_id GROUP BY customers.id HAVING total_qty >= 2 ORDER BY total_qty DESC;"},checklist:["Show customer name, total quantity, distinct products","Only customers with at least 2 items","Sort by quantity descending"]}];var N=[{id:"31-subquery-where",module:7,title:"Subquery in WHERE",type:"practice",file:"31-subquery-where.sql",markdown:`# Subquery in WHERE
...
**Goal:** Return names of customers who placed orders worth more than 100. Use a subquery with \`WHERE id IN\`.`,seed:t,check:{type:"result",expectedSql:"SELECT name FROM customers WHERE id IN (SELECT customer_id FROM orders WHERE price > 100);"}},{id:"32-subquery-select",module:7,title:"Subquery in SELECT",type:"practice",file:"32-subquery-select.sql",markdown:`# Subquery in SELECT
...
**Goal:** Show each customer name alongside the number of orders they placed.`,seed:t,check:{type:"result",expectedSql:"SELECT name, (SELECT COUNT(*) FROM orders WHERE customer_id = customers.id) AS order_count FROM customers;"}},{id:"33-subquery-from",module:7,title:"Subquery in FROM",type:"practice",file:"33-subquery-from.sql",markdown:`# Subquery in FROM
...
**Goal:** Find all expensive items (price > 50) by querying from a subquery that selects all orders.`,seed:t,check:{type:"result",expectedSql:"SELECT item, price FROM (SELECT * FROM orders) AS expensive WHERE price > 50;"}},{id:"34-correlated",module:7,title:"Correlated Subqueries",type:"practice",file:"34-correlated.sql",markdown:`# Correlated Subqueries
...
**Goal:** Return items from \`orders\` that cost more than the average price.`,seed:t,check:{type:"result",expectedSql:"SELECT item, price FROM orders WHERE price > (SELECT AVG(price) FROM orders);"}},{id:"35-exists",module:7,title:"EXISTS",type:"practice",file:"35-exists.sql",markdown:`# EXISTS
...
**Goal:** Use a CTE called \`avg_price\` to calculate the average price, then find all items above that average.`,seed:t,check:{type:"result",expectedSql:"WITH avg_price AS (SELECT AVG(price) AS avg FROM orders) SELECT item, price FROM orders, avg_price WHERE price > avg_price.avg;"}},{id:"37-recursive-cte",module:7,title:"Recursive CTEs (Theory)",type:"theory",file:"37-recursive-cte.md",markdown:`# Recursive CTEs
...
- [ ] Create \`orders\` (id, customer_id, product, price)`,seed:i,check:{type:"schema",table:"customers",columns:["id","name","city"]},checklist:["Table customers with id, name, city","Table orders with id, customer_id, product, price"]},{id:"40-2nf",module:8,title:"Second Normal Form (2NF)",type:"practice",file:"40-2nf.sql",markdown:`# Second Normal Form
...
- [ ] Create \`orders\` (id, customer_id, product_id)`,seed:i,check:{type:"schema",table:"products",columns:["id","name","category"]},checklist:["Table customers with id, name, city","Table products with id, name, category","Table orders with id, customer_id, product_id"]},{id:"41-3nf",module:8,title:"Third Normal Form (3NF)",type:"practice",file:"41-3nf.sql",markdown:"# Third Normal Form\n\nA table is in 3NF when:\n- It is in 2NF\n- No transitive dependency (column depends on another non-key column)\n\nIn CashPal's data, `customer_city` depends on `customer`, not the order. We've already split this.\n\n**Your task:**\n- [ ] Create `customers` (id, name, city)\n- [ ] Create `orders` (id, customer_id, product, price)\n- [ ] Add FOREIGN KEY on `customer_id` \u2192 `customers(id)`",seed:i,check:{type:"fk",table:"orders",column:"customer_id"},checklist:["Table customers with id, name, city","Table orders with id, customer_id, product, price","Foreign key on customer_id referencing customers(id)"]},{id:"42-denormalization",module:8,title:"Denormalization",type:"theory",file:"42-denormalization.md",markdown:`# Denormalization
...
**Goal:** know when to denormalize.`,question:{prompt:"When is denormalization useful?",options:["Always","When data must be unique","Never","When read performance matters more than write efficiency"],answer:3,explanation:"Denormalization speeds up reads by reducing joins, at the cost of redundant data."},seed:i}];var S=[{id:"43-what-index",module:9,title:"What is an Index?",type:"theory",file:"43-what-index.md",markdown:`# What is an Index?
...
**Goal:** Run EXPLAIN QUERY PLAN on \`SELECT * FROM products WHERE category = 'Electronics'\`.`,seed:o,check:{type:"success"}},{id:"46-composite-index",module:9,title:"Composite Indexes",type:"practice",file:"46-composite-index.sql",markdown:"# Composite Indexes\n\nA composite index covers multiple columns. Column order matters \u2014 leftmost first.\n\n**Your task:**\n- [ ] Create composite index `idx_cat_price` on `products(category, price)`\n- [ ] Create index `idx_stock` on `products(stock)`",seed:o,check:{type:"changes",min:0},checklist:["Composite index idx_cat_price on (category, price)","Index idx_stock on (stock)"]},{id:"47-no-index",module:9,title:"When NOT to Index",type:"theory",file:"47-no-index.md",markdown:`# When NOT to Index
...
**Goal:** know when indexes hurt more than help.`,question:{prompt:"Which column is a BAD candidate for an index?",options:["A primary key","A column with many unique values","A column with only two possible values","A foreign key"],answer:2,explanation:"Low-selectivity columns (few unique values) make poor indexes."},seed:o}];var w=[{id:"48-acid",module:10,title:"ACID Properties",type:"theory",file:"48-acid.md",markdown:`# ACID Properties
...
**Goal:** Create \`tasks\` (id INTEGER PK, title TEXT NOT NULL) inside a transaction, then COMMIT.`,seed:"",check:{type:"schema",table:"tasks",columns:["id","title"]}},{id:"50-commit",module:10,title:"Committing",type:"practice",file:"50-commit.sql",markdown:`# Committing

COMMIT saves all changes since BEGIN. Changes become permanent.

**Goal:** Insert a row into \`tasks\` inside a transaction and COMMIT.`,seed:"",check:{type:"changes",min:1}},{id:"51-rollback",module:10,title:"Rolling Back",type:"practice",file:"51-rollback.sql",markdown:"# Rolling Back\n\nROLLBACK undoes all changes since BEGIN \u2014 CashPal's safety net.\n\n```sql\nBEGIN;\nDELETE FROM table;\nROLLBACK; -- nothing happened\n```\n\n**Goal:** Delete all rows from `tasks` inside a transaction, then ROLLBACK. Verify with `SELECT COUNT(*)`.",seed:"",check:{type:"success"}},{id:"52-savepoint",module:10,title:"Savepoints",type:"practice",file:"52-savepoint.sql",markdown:`# Savepoints
...
**Goal:** Insert two rows into \`tasks\` after a SAVEPOINT, ROLLBACK TO that savepoint, insert one more row, and COMMIT. Only the last row persists.`,seed:"",check:{type:"changes",min:1}}];var I=[{id:"53-views",module:11,title:"Views",type:"practice",file:"53-views.sql",markdown:`# Views
...
**Goal:** Create a view \`customer_orders\` showing customer names and their order items (use INNER JOIN).`,seed:t,check:{type:"success"}},{id:"54-triggers",module:11,title:"Triggers",type:"practice",file:"54-triggers.sql",markdown:`# Triggers
...
**Goal:** Create trigger \`prevent_empty\` that prevents deleting the last product in any category. Use \`BEFORE DELETE\` with \`RAISE(ABORT, '...')\`.`,seed:o,check:{type:"success"}},{id:"55-window",module:11,title:"Window Functions",type:"practice",file:"55-window.sql",markdown:`# Window Functions
...
**Goal:** Return each product name, price, and a row number ordered by price descending (most expensive first).`,seed:o,check:{type:"result",expectedSql:"SELECT name, price, ROW_NUMBER() OVER (ORDER BY price DESC) AS rank FROM products;"}},{id:"56-case",module:11,title:"CASE Statements",type:"practice",file:"56-case.sql",markdown:`# CASE Statements
...
- [ ] Sort by price ascending`,seed:o,check:{type:"result",expectedSql:"SELECT name, CASE WHEN price < 100 THEN 'Cheap' WHEN price BETWEEN 100 AND 500 THEN 'Moderate' ELSE 'Expensive' END AS label FROM products ORDER BY price;"},checklist:["Label Cheap for price < 100","Label Moderate for price 100-500","Label Expensive for price > 500","Sort by price ascending"]},{id:"57-datetime",module:11,title:"Date and Time Functions",type:"practice",file:"57-datetime.sql",markdown:`# Date and Time Functions
...
**Goal:** Return event names and their month number (1-12) from \`events\`. Use \`STRFTIME('%m', event_date) AS month\`. Sort by event_date.`,seed:E,check:{type:"result",expectedSql:"SELECT name, STRFTIME('%m', event_date) AS month FROM events ORDER BY event_date;"}},{id:"58-capstone",module:11,title:"Final Capstone",type:"practice",file:"58-capstone.sql",markdown:`# Final Capstone
...
**Goal:** know when to denormalize and the trade-offs.`,question:{prompt:"When is denormalization useful?",options:["Always","When read performance matters more than write efficiency","Never","When data must be unique"],answer:1,explanation:"Denormalization speeds up reads at the cost of redundancy."},seed:""}];var ne={1:"Relational Database Theory",2:"Database Fundamentals",3:"Schema & Constraints",4:"CRUD Operations",5:"Query Power Tools",6:"Joins",7:"Subqueries & CTEs",8:"Normalization",9:"Indexes & Performance",10:"Transactions",11:"Advanced Topics"};function s(b){return b.map((r,c)=>({...r,_order:r.order??c})).sort((r,c)=>r._order-c._order)}var ie=[...A,...s(p),...s(h),...s(T),...s(y),...s(R),...s(N),...s(L),...s(S),...s(w),...s(I)];export{e as a,ne as b,ie as c};
