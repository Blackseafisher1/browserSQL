...
**Goal:** Write a query that shows each customer name alongside the number of orders they placed. Use a subquery in SELECT with \`COUNT(*)\`.`,seed:t,check:{type:"result",expectedSql:"SELECT name, (SELECT COUNT(*) FROM orders WHERE customer_id = customers.id) AS order_count FROM customers;"}},{id:"33-subquery-from",module:6,title:"Subquery in FROM",type:"practice",file:"33-subquery-from.sql",markdown:"# Subquery in FROM\n\nA subquery in `FROM` acts like a temporary table. It must have an alias:\n\n```sql\nSELECT columns FROM (SELECT ...) AS alias WHERE condition;\n```\n\n**Goal:** Write a query that finds all expensive items (price > 50) by querying from a subquery that selects all orders. Use `FROM (SELECT * FROM orders) AS expensive` and filter with WHERE.",seed:t,check:{type:"result",expectedSql:"SELECT item, price FROM (SELECT * FROM orders) AS expensive WHERE price > 50;"}},{id:"34-correlated",module:6,title:"Correlated Subqueries",type:"practice",file:"34-correlated.sql",markdown:`# Correlated subqueries
...
**Goal:** Write a query that returns items from \`orders\` that cost more than the average price across all orders.`,seed:t,check:{type:"result",expectedSql:"SELECT item, price FROM orders WHERE price > (SELECT AVG(price) FROM orders);"}},{id:"35-exists",module:6,title:"EXISTS",type:"practice",file:"35-exists.sql",markdown:"# EXISTS\n\n`EXISTS` checks whether a subquery returns any rows. It is often faster than `IN`:\n\n```sql\nSELECT columns FROM table_a AS a\nWHERE EXISTS (SELECT 1 FROM table_b WHERE b.ref_id = a.id);\n```\n\n**Goal:** Write a query that returns the names of customers who have placed at least one order. Use `EXISTS`.",seed:t,check:{type:"result",expectedSql:"SELECT name FROM customers WHERE EXISTS (SELECT 1 FROM orders WHERE customer_id = customers.id);"}},{id:"36-cte",module:6,title:"Common Table Expressions",type:"practice",file:"36-cte.sql",markdown:`# Common Table Expressions
