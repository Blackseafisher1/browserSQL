...
INSERT INTO events VALUES
  ...
  (5, 'Review', '2025-03-01');
`;var p=[{id:"01-intro",module:1,title:"What is a Database?",type:"theory",file:"01-intro.md",markdown:`# What is a Database?

Welcome to **CashPal** \u2014 a fast-growing fintech startup. As their first data engineer, you'll build everything from scratch.

A database stores information in tables. Tables have **rows** (records) and **columns** (fields). Think of it like a spreadsheet, but more powerful.

**In this course you will:**
- Write real SQL queries
- Build tables and relationships
- Query, filter, and join data

**Goal:** understand where data lives before writing SQL.

> **Editor features:** The left panel shows files. The **Schema viewer** (bottom-left) lists all tables \u2014 expand one to see columns, types, and constraints. Click any table name to auto-run \`SELECT * FROM table LIMIT 100\`.`,question:{prompt:"What stores data in rows and columns?",options:["Index","Query","Table","View"],answer:2,explanation:"A table is the structure that holds rows and columns."},seed:e},{id:"02-nosql",module:1,title:"SQL vs NoSQL",type:"theory",file:"02-nosql.md",markdown:`# SQL vs NoSQL

CashPal needs a reliable system for transactions, users, and accounts. **SQL databases** use tables and enforce a structured schema \u2014 perfect for financial data that must be consistent.

**NoSQL** systems (document stores, key-value, graph) are more flexible but don't enforce relationships. Great for rapid prototyping, bad for accounting.

**Goal:** know which keyword creates tables in SQL.`,question:{prompt:"Which SQL keyword creates a table?",options:["CREATE TABLE","ALTER","INSERT","UPDATE"],answer:0,explanation:"CREATE TABLE defines a new table structure."},seed:e},{id:"03-comments",module:1,title:"Comments in SQL",type:"theory",file:"03-comments.md",markdown:`# Comments in SQL

Comments make SQL readable. They are ignored when the query runs:
...
CashPal's codebase requires comments on all queries. Get used to writing them.
...
**Goal:** Write a query that returns 100 divided by 4.`,check:{type:"result",expectedSql:"SELECT 100 / 4;"}}];var T=[{id:"06-primary",module:2,title:"PRIMARY KEY",type:"theory",file:"06-primary.md",markdown:`# PRIMARY KEY
...
**Goal:** understand what PRIMARY KEY guarantees.`,question:{prompt:"What does PRIMARY KEY automatically imply?",options:["NOT NULL only","UNIQUE only","NOT NULL + UNIQUE","AUTOINCREMENT"],answer:2,explanation:"PRIMARY KEY implies both NOT NULL and UNIQUE."},seed:""},{id:"10-constraints",module:2,title:"Constraints",type:"practice",file:"10-constraints.sql",sql:`CREATE TABLE accounts (
  id INTEGER PRIMARY KEY,
...
**Goal:** Create \`authors\` (id, name) and \`books\` (id, title, author_id FK).`,seed:i,check:{type:"fk",table:"books",column:"author_id"},hint:"CREATE TABLE authors (id INTEGER PRIMARY KEY, name TEXT); CREATE TABLE books (id INTEGER PRIMARY KEY, title TEXT, author_id INTEGER REFERENCES authors(id));"},{id:"11-relationship-1-1",module:2,title:"One-to-One Relationships",type:"practice",file:"11-relationship-1-1.sql",markdown:`# One-to-One Relationships
...
**Goal:** Create \`citizens\` (id, name) and \`passports\` (id PK referencing citizens, number TEXT).`,seed:"",check:{type:"fk",table:"passports",column:"id"},hint:"CREATE TABLE citizens (id INTEGER PRIMARY KEY, name TEXT); CREATE TABLE passports (id INTEGER PRIMARY KEY, number TEXT, FOREIGN KEY (id) REFERENCES citizens(id));"},{id:"09-cascade",module:2,title:"ON DELETE CASCADE",type:"theory",file:"09-cascade.md",markdown:`# ON DELETE CASCADE
...
- [ ] Add \`quantity\` column to order_items`,seed:i,check:{type:"constraints",table:"order_items",tokens:["primary key"]},hint:"CREATE TABLE orders (id INTEGER PRIMARY KEY, date TEXT); CREATE TABLE products (id INTEGER PRIMARY KEY, name TEXT); CREATE TABLE order_items (order_id INTEGER, product_id INTEGER, quantity INTEGER, PRIMARY KEY (order_id, product_id));",checklist:["Table orders with id and date","Table products with id and name","Table order_items with order_id, product_id, quantity","Composite PRIMARY KEY on (order_id, product_id)"]},{id:"14-relationship-n-m",module:2,title:"Many-to-Many Relationships",type:"practice",file:"14-relationship-n-m.sql",markdown:"# Many-to-Many Relationships\n\nN:M requires a junction table. The junction table has a composite PK to prevent duplicates.\n\nCashPal's content system: an actor can be in many movies, a movie has many actors.\n\n**Goal:** Create `actors`, `movies`, and a junction table `cast` with composite PK on (actor_id, movie_id).",seed:i,check:{type:"constraints",table:"cast",tokens:["primary key"]},hint:"CREATE TABLE actors (id INTEGER PRIMARY KEY, name TEXT); CREATE TABLE movies (id INTEGER PRIMARY KEY, title TEXT); CREATE TABLE cast (actor_id INTEGER, movie_id INTEGER, PRIMARY KEY (actor_id, movie_id), FOREIGN KEY (actor_id) REFERENCES actors(id), FOREIGN KEY (movie_id) REFERENCES movies(id));"},{id:"09-alter-table",module:2,title:"ALTER TABLE",type:"practice",file:"09-alter-table.sql",markdown:"# ALTER TABLE\n\nCashPal's `users` table needs a phone number field. You can modify existing tables with `ALTER TABLE`:\n\n```sql\nALTER TABLE table ADD COLUMN column TYPE;\nALTER TABLE table RENAME COLUMN old TO new;\nALTER TABLE table DROP COLUMN column;\n```\n\nThe `users` table has: id, name, city, age, email.\n\n**Your task:**\n- [ ] Add column `phone` (TEXT) to `users`\n- [ ] Rename `phone` to `phone_number`",seed:e,check:{type:"schema",table:"users",columns:["id","name","city","age","email","phone_number"]},hint:"ALTER TABLE users ADD COLUMN phone TEXT; then ALTER TABLE users RENAME COLUMN phone TO phone_number;",checklist:["Add column phone (TEXT) to users","Rename phone to phone_number"]},{id:"15-drop-table",module:2,title:"DROP TABLE",type:"practice",file:"11-drop-table.sql",markdown:"# DROP TABLE\n\n`DROP TABLE` removes a table and all its data permanently. Use with caution.\n\n```sql\nDROP TABLE table_name;\nDROP TABLE IF EXISTS table_name;  -- safe version\n```\n\n**Goal:** Drop the `users` table. Then run `SELECT * FROM sqlite_master WHERE type='table'` to verify it's gone.",seed:e,check:{type:"success"},hint:"DROP TABLE users;"},{id:"10-schema",module:2,title:"Schema Design",type:"theory",file:"10-schema.md",markdown:`# Schema Design
...
`,markdown:"# Reading Data\n\nCashPal's `users` table is ready. Let's read from it.\n\n```sql\nSELECT column1, column2 FROM tablename;\nSELECT * FROM tablename;  -- all columns\n```\n\n**Goal:** Write a query that returns all rows and columns from the `users` table.",seed:e,check:{type:"result",expectedSql:"SELECT * FROM users;"}},{id:"14-where",module:3,title:"Filtering",type:"practice",file:"11-where.sql",sql:`SELECT name FROM users WHERE city = 'Berlin';
`,markdown:`# Filtering
...
`,markdown:"# Inserting Data\n\nA new user joined CashPal. Add them to the database.\n\n```sql\nINSERT INTO tablename (col1, col2, ...) VALUES (val1, val2, ...);\n```\n\nThe `id` column is auto-incrementing \u2014 you can omit it.\n\n**Goal:** Insert a new user into `users`.",seed:e,check:{type:"changes",min:1}},{id:"19-update",module:3,title:"Updating Data",type:"practice",file:"16-update.sql",sql:`UPDATE users SET city = 'Bremen' WHERE name = 'Mia';
...
**Goal:** Update the city of a specific user.`,seed:e,check:{type:"changes",min:1}},{id:"20-update-multi",module:3,title:"UPDATE Multiple Columns",type:"practice",file:"20-update-multi.sql",markdown:"# Update Multiple Columns\n\nA CashPal user changed both city and age. Update both at once:\n\n```sql\nUPDATE users SET city = 'Berlin', age = 30 WHERE name = 'Mia';\n```\n\n**Goal:** Update both the city and age of a user in `users`.",seed:e,check:{type:"changes",min:1},hint:"UPDATE users SET city = 'X', age = Y WHERE name = 'Z';"},{id:"21-delete",module:3,title:"Deleting Data",type:"practice",file:"21-delete.sql",sql:`DELETE FROM users WHERE name = 'Liam';
...
- [ ] Delete 'Keyboard'
- [ ] SELECT remaining items sorted by item name`,seed:"",check:{type:"result",expectedSql:"SELECT item, quantity FROM inventory ORDER BY item;"},checklist:["Create table inventory with id, item, quantity","Insert Laptop (5), Mouse (20), Keyboard (15)","Update Mouse quantity to 25","Delete Keyboard","SELECT remaining items sorted by name"]},{id:"25-insert-multi",module:3,title:"INSERT Multiple Rows",type:"practice",file:"25-insert-multi.sql",markdown:`# INSERT Multiple Rows
...
**Goal:** Return the names of the 3 oldest users, sorted oldest first.`,seed:e,check:{type:"result",expectedSql:"SELECT name FROM users ORDER BY age DESC LIMIT 3;"}},{id:"21-aggregates",module:4,title:"Aggregate Functions",type:"practice",file:"21-aggregates.sql",markdown:`# Aggregate Functions
...
**Goal:** Return the total number of users and their average age rounded to 2 decimals.`,seed:l,check:{type:"result",expectedSql:"SELECT COUNT(*), ROUND(AVG(age), 2) FROM users;"},hint:"SELECT COUNT(*), ROUND(AVG(age), 2) FROM users;"},{id:"22-group",module:4,title:"Grouping",type:"practice",file:"22-group.sql",markdown:`# Grouping
...
- [ ] Sort alphabetically`,seed:l,check:{type:"result",expectedSql:"SELECT city, COUNT(*) FROM users GROUP BY city HAVING COUNT(*) >= 2 ORDER BY city;"},hint:"GROUP BY city, then HAVING COUNT(*) >= 2",checklist:["Count users per city","Filter cities with at least 2 users","Sort alphabetically"]},{id:"23-distinct",module:4,title:"Distinct Values",type:"practice",file:"23-distinct.sql",markdown:"# Distinct Values\n\nCashPal wants a list of unique cities \u2014 no duplicates.\n\n`DISTINCT` removes duplicate values:\n\n```sql\nSELECT DISTINCT column FROM table;\n```\n\n**Goal:** Return all unique cities from `users`.",seed:e,check:{type:"result",expectedSql:"SELECT DISTINCT city FROM users;"}},{id:"24-alias",module:4,title:"Aliases",type:"practice",file:"24-alias.sql",markdown:"# Aliases\n\nCashPal's API needs specific column names in the response.\n\n`AS` renames columns in results:\n\n```sql\nSELECT column AS alias_name FROM table;\n```\n\n**Goal:** Return `name` as `user_name` and `age` as `user_age` from `users`.",seed:e,check:{type:"result",expectedSql:"SELECT name AS user_name, age AS user_age FROM users;"}},{id:"25-query-mastery",module:4,title:"Query Mastery",type:"practice",file:"25-query-mastery.sql",markdown:`# Query Mastery
...
**Goal:** Show each customer name alongside their order item. Join on \`customers.id = orders.customer_id\`.`,seed:t,check:{type:"result",expectedSql:"SELECT customers.name, orders.item FROM customers INNER JOIN orders ON customers.id = orders.customer_id;"}},{id:"26-old-join",module:5,title:"Old-School Joins (Implicit)",type:"theory",file:"26-old-join.md",markdown:`# Old-School Implicit Joins
...
**Goal:** Show each employee name alongside their manager's name. Use LEFT JOIN so top-level employees (no manager) still appear.`,seed:d,check:{type:"result",expectedSql:"SELECT e.name AS employee, m.name AS manager FROM employees e LEFT JOIN employees m ON e.manager_id = m.id;"}},{id:"30-multi-join",module:5,title:"Joining Multiple Tables",type:"practice",file:"30-multi-join.sql",markdown:`# Joining Multiple Tables
...
**Goal:** Show customer name, product name, and order quantity by joining \`customers\`, \`orders\`, and \`products\`.`,seed:E,check:{type:"result",expectedSql:"SELECT customers.name, products.name, orders.quantity FROM customers INNER JOIN orders ON customers.id = orders.customer_id INNER JOIN products ON orders.product_id = products.id;"}},{id:"31-join-mastery",module:5,title:"Join Mastery",type:"practice",file:"31-join-mastery.sql",markdown:`# Join Mastery
...
- [ ] Sort by total quantity descending`,seed:E,check:{type:"result",expectedSql:"SELECT customers.name, SUM(orders.quantity) AS total_qty, COUNT(DISTINCT orders.product_id) AS distinct_products FROM customers INNER JOIN orders ON customers.id = orders.customer_id GROUP BY customers.id HAVING total_qty >= 2 ORDER BY total_qty DESC;"},checklist:["Show customer name, total quantity, distinct products","Only customers with at least 2 items","Sort by quantity descending"]}];var N=[{id:"31-subquery-where",module:6,title:"Subquery in WHERE",type:"practice",file:"31-subquery-where.sql",markdown:`# Subquery in WHERE
...
**Goal:** Return names of customers who placed orders worth more than 100. Use a subquery with \`WHERE id IN\`.`,seed:t,check:{type:"result",expectedSql:"SELECT name FROM customers WHERE id IN (SELECT customer_id FROM orders WHERE price > 100);"}},{id:"32-subquery-select",module:6,title:"Subquery in SELECT",type:"practice",file:"32-subquery-select.sql",markdown:`# Subquery in SELECT
...
**Goal:** Show each customer name alongside the number of orders they placed.`,seed:t,check:{type:"result",expectedSql:"SELECT name, (SELECT COUNT(*) FROM orders WHERE customer_id = customers.id) AS order_count FROM customers;"}},{id:"33-subquery-from",module:6,title:"Subquery in FROM",type:"practice",file:"33-subquery-from.sql",markdown:`# Subquery in FROM
...
**Goal:** Find all expensive items (price > 50) by querying from a subquery that selects all orders.`,seed:t,check:{type:"result",expectedSql:"SELECT item, price FROM (SELECT * FROM orders) AS expensive WHERE price > 50;"}},{id:"34-correlated",module:6,title:"Correlated Subqueries",type:"practice",file:"34-correlated.sql",markdown:`# Correlated Subqueries
...
**Goal:** Return items from \`orders\` that cost more than the average price.`,seed:t,check:{type:"result",expectedSql:"SELECT item, price FROM orders WHERE price > (SELECT AVG(price) FROM orders);"}},{id:"35-exists",module:6,title:"EXISTS",type:"practice",file:"35-exists.sql",markdown:`# EXISTS
...
**Goal:** Return names of customers who have placed at least one order. Use EXISTS.`,seed:t,check:{type:"result",expectedSql:"SELECT name FROM customers WHERE EXISTS (SELECT 1 FROM orders WHERE customer_id = customers.id);"}},{id:"36-cte",module:6,title:"Common Table Expressions",type:"practice",file:"36-cte.sql",markdown:`# Common Table Expressions
...
**Goal:** know when recursive CTEs are useful.`,question:{prompt:"What kind of data is a recursive CTE best for?",options:["Flat tables","Hierarchical data like org charts","Single-row results","Aggregated data"],answer:1,explanation:"Recursive CTEs excel at querying tree structures like employee hierarchies."},seed:d}];var L=[{id:"38-why-normalize",module:7,title:"Why Normalize?",type:"theory",file:"38-why-normalize.md",markdown:`# Why Normalize?
...
- [ ] Create \`orders\` (id, customer_id, product, price)`,seed:n,check:{type:"schema",table:"customers",columns:["id","name","city"]},checklist:["Table customers with id, name, city","Table orders with id, customer_id, product, price"]},{id:"40-2nf",module:7,title:"Second Normal Form (2NF)",type:"practice",file:"40-2nf.sql",markdown:`# Second Normal Form
...
- [ ] Create \`orders\` (id, customer_id, product_id)`,seed:n,check:{type:"schema",table:"products",columns:["id","name","category"]},checklist:["Table customers with id, name, city","Table products with id, name, category","Table orders with id, customer_id, product_id"]},{id:"41-3nf",module:7,title:"Third Normal Form (3NF)",type:"practice",file:"41-3nf.sql",markdown:"# Third Normal Form\n\nA table is in 3NF when:\n- It is in 2NF\n- No transitive dependency (column depends on another non-key column)\n\nIn CashPal's data, `customer_city` depends on `customer`, not the order. We've already split this.\n\n**Your task:**\n- [ ] Create `customers` (id, name, city)\n- [ ] Create `orders` (id, customer_id, product, price)\n- [ ] Add FOREIGN KEY on `customer_id` \u2192 `customers(id)`",seed:n,check:{type:"fk",table:"orders",column:"customer_id"},checklist:["Table customers with id, name, city","Table orders with id, customer_id, product, price","Foreign key on customer_id referencing customers(id)"]},{id:"42-denormalization",module:7,title:"Denormalization",type:"theory",file:"42-denormalization.md",markdown:`# Denormalization
...
**Goal:** know when to denormalize.`,question:{prompt:"When is denormalization useful?",options:["Always","When data must be unique","Never","When read performance matters more than write efficiency"],answer:3,explanation:"Denormalization speeds up reads by reducing joins, at the cost of redundant data."},seed:n}];var S=[{id:"43-what-index",module:8,title:"What is an Index?",type:"theory",file:"43-what-index.md",markdown:`# What is an Index?
...
**Goal:** know what an index does.`,question:{prompt:"Which columns are indexed automatically in SQLite?",options:["All columns","PRIMARY KEY columns","TEXT columns","No columns"],answer:1,explanation:"PRIMARY KEY columns get an automatic index."},seed:s},{id:"44-create-index",module:8,title:"Creating Indexes",type:"practice",file:"44-create-index.sql",markdown:"# Creating Indexes\n\nCashPal's `products` table needs faster category lookups.\n\nUse `CREATE INDEX` to add an index:\n\n```sql\nCREATE INDEX index_name ON table (column);\n```\n\n**Goal:** Create an index `idx_category` on `products(category)`.",seed:s,check:{type:"success"}},{id:"45-explain-plan",module:8,title:"Query Planning",type:"practice",file:"45-explain-plan.sql",markdown:`# Query Planning
...
**Goal:** Run EXPLAIN QUERY PLAN on \`SELECT * FROM products WHERE category = 'Electronics'\`.`,seed:s,check:{type:"success"}},{id:"46-composite-index",module:8,title:"Composite Indexes",type:"practice",file:"46-composite-index.sql",markdown:"# Composite Indexes\n\nA composite index covers multiple columns. Column order matters \u2014 leftmost first.\n\n**Your task:**\n- [ ] Create composite index `idx_cat_price` on `products(category, price)`\n- [ ] Create index `idx_stock` on `products(stock)`",seed:s,check:{type:"changes",min:0},checklist:["Composite index idx_cat_price on (category, price)","Index idx_stock on (stock)"]},{id:"47-no-index",module:8,title:"When NOT to Index",type:"theory",file:"47-no-index.md",markdown:`# When NOT to Index
...
**Goal:** know when indexes hurt more than help.`,question:{prompt:"Which column is a BAD candidate for an index?",options:["A primary key","A column with many unique values","A column with only two possible values","A foreign key"],answer:2,explanation:"Low-selectivity columns (few unique values) make poor indexes."},seed:s}];var I=[{id:"48-acid",module:9,title:"ACID Properties",type:"theory",file:"48-acid.md",markdown:`# ACID Properties
...
**Goal:** know what ACID stands for.`,question:{prompt:"What does the I in ACID stand for?",options:["Index","Isolation","Integrity","Insert"],answer:1,explanation:"Isolation ensures concurrent transactions do not interfere."},seed:""},{id:"49-begin",module:9,title:"Starting Transactions",type:"practice",file:"49-begin.sql",markdown:`# Starting Transactions
...
**Goal:** Create \`tasks\` (id INTEGER PK, title TEXT NOT NULL) inside a transaction, then COMMIT.`,seed:"",check:{type:"schema",table:"tasks",columns:["id","title"]}},{id:"50-commit",module:9,title:"Committing",type:"practice",file:"50-commit.sql",markdown:`# Committing

COMMIT saves all changes since BEGIN. Changes become permanent.

**Goal:** Insert a row into \`tasks\` inside a transaction and COMMIT.`,seed:"",check:{type:"changes",min:1}},{id:"51-rollback",module:9,title:"Rolling Back",type:"practice",file:"51-rollback.sql",markdown:"# Rolling Back\n\nROLLBACK undoes all changes since BEGIN \u2014 CashPal's safety net.\n\n```sql\nBEGIN;\nDELETE FROM table;\nROLLBACK; -- nothing happened\n```\n\n**Goal:** Delete all rows from `tasks` inside a transaction, then ROLLBACK. Verify with `SELECT COUNT(*)`.",seed:"",check:{type:"success"}},{id:"52-savepoint",module:9,title:"Savepoints",type:"practice",file:"52-savepoint.sql",markdown:`# Savepoints
...
**Goal:** Insert two rows into \`tasks\` after a SAVEPOINT, ROLLBACK TO that savepoint, insert one more row, and COMMIT. Only the last row persists.`,seed:"",check:{type:"changes",min:1}}];var C=[{id:"53-views",module:10,title:"Views",type:"practice",file:"53-views.sql",markdown:`# Views
...
**Goal:** Create a view \`customer_orders\` showing customer names and their order items (use INNER JOIN).`,seed:t,check:{type:"success"}},{id:"54-triggers",module:10,title:"Triggers",type:"practice",file:"54-triggers.sql",markdown:`# Triggers
...
**Goal:** Create trigger \`prevent_empty\` that prevents deleting the last product in any category. Use \`BEFORE DELETE\` with \`RAISE(ABORT, '...')\`.`,seed:s,check:{type:"success"}},{id:"55-window",module:10,title:"Window Functions",type:"practice",file:"55-window.sql",markdown:`# Window Functions
...
- [ ] Sort by price ascending`,seed:s,check:{type:"result",expectedSql:"SELECT name, CASE WHEN price < 100 THEN 'Cheap' WHEN price BETWEEN 100 AND 500 THEN 'Moderate' ELSE 'Expensive' END AS label FROM products ORDER BY price;"},checklist:["Label Cheap for price < 100","Label Moderate for price 100-500","Label Expensive for price > 500","Sort by price ascending"]},{id:"57-datetime",module:10,title:"Date and Time Functions",type:"practice",file:"57-datetime.sql",markdown:`# Date and Time Functions
...
- [ ] Query: show currently loaned books (returned = 0) with member name and book title \u2014 use JOIN`,seed:i,check:{type:"schema",table:"loans",columns:["id","member_id","book_id","loan_date","returned"]},checklist:["Table members with id, name, joined_date","Table books with id, title, author","Table loans with id, member_id, book_id, loan_date, returned","Insert 2 members, 3 books, 2 loans","Query shows loaned books with member name and book title"]}];var ae={1:"Database Fundamentals",2:"Schema & Constraints",3:"CRUD Operations",4:"Query Power Tools",5:"Joins",6:"Subqueries & CTEs",7:"Normalization",8:"Indexes & Performance",9:"Transactions",10:"Advanced Topics"};function o(O){return O.map((r,c)=>({...r,_order:r.order??c})).sort((r,c)=>r._order-c._order)}var se=[...o(p),...o(T),...o(h),...o(R),...o(y),...o(N),...o(L),...o(S),...o(I),...o(C)];export{e as a,ae as b,se as c};
