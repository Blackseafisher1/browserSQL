...
const SQLKeywords = "absolute action add after all allocate alter and any are as asc assertion at authorization before begin between both breadth by call cascade cascaded case cast catalog check close collate collation column commit condition connect connection constraint constraints constructor continue corresponding count create cross cube current current_date current_default_transform_group current_transform_group_for_type current_path current_role current_time current_timestamp current_user cursor cycle data day deallocate declare default deferrable deferred delete depth deref desc describe descriptor deterministic diagnostics disconnect distinct do domain drop dynamic each else elseif end end-exec exclude end-exec equals escape except exception exec execute exists exit external fetch filter first following for foreign found from free full function general get global go goto grant group grouping groups handle having hold hour identity if ignore immediate in indicator initially inner inout input insert intersect into is isolation join key language last lateral leading leave left level like limit local localtime localtimestamp locator loop map match method minute modifies module month names natural nesting new next no none not nulls of old on only open option or order ordinality others out outer output over overlaps pad parameter partial partition path preceding prepare preserve primary prior privileges procedure public range read reads recursive redo ref references referencing relative release repeat resignal respect restrict result return returns revoke right role rollback rollup routine row rows savepoint schema scroll search second section select session session_user set sets signal similar size some space specific specifictype sql sqlexception sqlstate sqlwarning start state static system_user table temporary then ties timezone_hour timezone_minute to trailing transaction translation treat trigger unbounded under undo union unique unnest until update usage user using value values view when whenever where while window with without work write year zone ";
const defaults = {
...
let parser = /*@__PURE__*/parser$1.configure({
    props: [
        ...
        /*@__PURE__*/styleTags({
            ...
            "Semi": tags.keyword, "Punctuation": tags.punctuation,
            "( )": tags.paren,
            "{ }": tags.brace,
            "[ ]": tags.squareBracket
        })
