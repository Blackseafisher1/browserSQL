import { SEED_ABITUR, SEED_EMPLOYEES, SEED_SHOP_EXT, SEED_MARCH_ORDERS, SEED_NORDWIND } from './seeds.js';
...
export const DEFAULT_CHALLENGES = [
    ...
    tasks: [
      {
        ...
        seed: SEED_SALES, check: { type: 'result', expectedSql: 'SELECT id, mitarbeiter_id, umsatz, ROW_NUMBER() OVER (ORDER BY umsatz DESC) AS rang FROM verkäufe' },
        sql: 'SELECT id, mitarbeiter_id, umsatz, ROW_NUMBER() OVER (ORDER BY umsatz DESC) AS rang FROM verkäufe;', hint: 'ROW_NUMBER() OVER (ORDER BY ...)',
      ...
      },
      {
        ...
        seed: SEED_SALES, check: { type: 'result', expectedSql: 'SELECT id, umsatz, RANK() OVER (ORDER BY umsatz DESC) AS rang FROM verkäufe' },
        sql: 'SELECT id, umsatz, RANK() OVER (ORDER BY umsatz DESC) AS rang FROM verkäufe;', hint: 'RANK() statt ROW_NUMBER()',
      ...
      },
      {
        ...
        seed: SEED_SALES, check: { type: 'result', expectedSql: 'SELECT mitarbeiter_id, umsatz, datum, ROW_NUMBER() OVER (PARTITION BY mitarbeiter_id ORDER BY umsatz DESC) AS mitarbeiter_rang FROM verkäufe' },
        sql: 'SELECT mitarbeiter_id, umsatz, datum, ROW_NUMBER() OVER (PARTITION BY mitarbeiter_id ORDER BY umsatz DESC) AS mitarbeiter_rang FROM verkäufe;', hint: 'ROW_NUMBER() OVER (PARTITION BY mitarbeiter_id ORDER BY ...)',
      ...
      },
      {
        ...
        seed: SEED_SALES, check: { type: 'result', expectedSql: "SELECT mitarbeiter_id, umsatz, datum, rang FROM (SELECT mitarbeiter_id, umsatz, datum, ROW_NUMBER() OVER (PARTITION BY mitarbeiter_id ORDER BY umsatz DESC) AS rang FROM verkäufe) WHERE rang <= 3" },
        sql: "SELECT mitarbeiter_id, umsatz, datum, rang FROM (SELECT mitarbeiter_id, umsatz, datum, ROW_NUMBER() OVER (PARTITION BY mitarbeiter_id ORDER BY umsatz DESC) AS rang FROM verkäufe) WHERE rang <= 3;", hint: 'Subquery mit ROW_NUMBER() im FROM, dann WHERE rang <= 3',
      ...
      },
      {
        ...
        seed: SEED_SALES, check: { type: 'result', expectedSql: 'SELECT datum, umsatz, SUM(umsatz) OVER (ORDER BY datum) AS kumuliert FROM verkäufe' },
        sql: 'SELECT datum, umsatz, SUM(umsatz) OVER (ORDER BY datum) AS kumuliert FROM verkäufe;', hint: 'SUM(umsatz) OVER (ORDER BY datum)',
      ...
      },
      {
        ...
        seed: SEED_SALES, check: { type: 'result', expectedSql: 'SELECT mitarbeiter_id, produkt_id, umsatz, AVG(umsatz) OVER (PARTITION BY mitarbeiter_id) AS avg_umsatz_mitarbeiter, umsatz - AVG(umsatz) OVER (PARTITION BY mitarbeiter_id) AS abweichung, ROW_NUMBER() OVER (PARTITION BY mitarbeiter_id ORDER BY umsatz DESC) AS rang_mitarbeiter FROM verkäufe ORDER BY mitarbeiter_id, umsatz DESC' },
        sql: 'SELECT mitarbeiter_id, produkt_id, umsatz, AVG(umsatz) OVER (PARTITION BY mitarbeiter_id) AS avg_umsatz_mitarbeiter, umsatz - AVG(umsatz) OVER (PARTITION BY mitarbeiter_id) AS abweichung, ROW_NUMBER() OVER (PARTITION BY mitarbeiter_id ORDER BY umsatz DESC) AS rang_mitarbeiter FROM verkäufe ORDER BY mitarbeiter_id, umsatz DESC;', hint: 'AVG() OVER (PARTITION BY ...) für den Durchschnitt, dann den Rang berechnen',
  ...
  {
    id: 'nordwind-joins',
    title: 'Joins mit der Nordwind-Datenbank',
    difficulty: 'medium',
    tags: ['JOIN', 'Nordwind', 'INNER JOIN', 'Mehr-Tabellen'],
    description: '10 JOIN-Aufgaben mit der Nordwind-Datenbank. Übe INNER JOINs über 2–5 Tabellen mit Lieferanten, Kategorien, Personal, Bestellungen und Artikeln.',
    defaultSeed: SEED_NORDWIND,
    tasks: [
      {
        id: 'nw-j-01', title: 'Kategorie von Teatime Chocolate Biscuits',
        markdown: 'Zu welcher Kategorie gehören die **Teatime Chocolate Biscuits**?\n\nVerwende einen JOIN zwischen `Artikel` und `Kategorie`.',
        seed: SEED_NORDWIND, check: { type: 'result', expectedSql: "SELECT k.Kategoriename FROM Artikel a JOIN Kategorie k ON a.KategorieNr = k.KategorieNr WHERE a.Artikelname = 'Teatime Chocolate Biscuits'" },
        sql: "SELECT k.Kategoriename FROM Artikel a JOIN Kategorie k ON a.KategorieNr = k.KategorieNr WHERE a.Artikelname = 'Teatime Chocolate Biscuits';", hint: 'JOIN Artikel mit Kategorie ON KategorieNr, dann WHERE Artikelname',
        checklist: ['JOIN zwischen Artikel und Kategorie', 'WHERE auf Artikelname', 'Ergebnis: Süßwaren'],
      },
      {
        id: 'nw-j-02', title: 'Artikel mit Lieferantenfirma',
        markdown: 'Erstelle eine Abfrage mit den Attributen **ArtikelNr**, **Artikelname** und **Lieferantenfirma**.\n\nJOIN zwischen `Artikel` und `Lieferant`.',
        seed: SEED_NORDWIND, check: { type: 'result', expectedSql: 'SELECT a.ArtikelNr, a.Artikelname, l.Firma FROM Artikel a JOIN Lieferant l ON a.LieferantenNr = l.LieferantenNr ORDER BY a.ArtikelNr' },
        sql: 'SELECT a.ArtikelNr, a.Artikelname, l.Firma FROM Artikel a JOIN Lieferant l ON a.LieferantenNr = l.LieferantenNr ORDER BY a.ArtikelNr;', hint: 'SELECT a.ArtikelNr, a.Artikelname, l.Firma FROM Artikel a JOIN Lieferant l ON ...',
        checklist: ['JOIN Artikel mit Lieferant', 'Drei Spalten: ArtikelNr, Artikelname, Firma'],
      },
      {
        id: 'nw-j-03', title: 'Versandfirma für Bestellung 10266',
        markdown: 'Über welche **Versandfirma** wurde die Bestellung **10266** ausgeliefert?\n\nJOIN zwischen `Bestellung` und `Versandfirma`.',
        seed: SEED_NORDWIND, check: { type: 'result', expectedSql: 'SELECT v.Firma FROM Bestellung b JOIN Versandfirma v ON b.FirmenNr = v.FirmenNr WHERE b.BestellNr = 10266' },
        sql: 'SELECT v.Firma FROM Bestellung b JOIN Versandfirma v ON b.FirmenNr = v.FirmenNr WHERE b.BestellNr = 10266;', hint: 'JOIN Bestellung mit Versandfirma ON FirmenNr, WHERE BestellNr = 10266',
        checklist: ['JOIN Bestellung mit Versandfirma', 'WHERE BestellNr', 'Ergebnis: Federal Shipping'],
      },
      {
        id: 'nw-j-04', title: 'Mitarbeiter für Bestellung 10266',
        markdown: 'Welcher **Mitarbeiter** ist für die Bestellung mit der Nummer **10266** zuständig?\n\nJOIN zwischen `Bestellung` und `Personal`. Gib Vor- und Nachname aus.',
        seed: SEED_NORDWIND, check: { type: 'result', expectedSql: "SELECT p.Vorname || ' ' || p.Nachname FROM Bestellung b JOIN Personal p ON b.PersonalNr = p.PersonalNr WHERE b.BestellNr = 10266" },
        sql: "SELECT p.Vorname || ' ' || p.Nachname AS mitarbeiter FROM Bestellung b JOIN Personal p ON b.PersonalNr = p.PersonalNr WHERE b.BestellNr = 10266;", hint: 'JOIN Bestellung mit Personal ON PersonalNr',
        checklist: ['JOIN Bestellung mit Personal', 'Vorname und Nachname kombiniert', 'Ergebnis: Janet Leverling'],
      },
      {
        id: 'nw-j-05', title: 'Vier-Tabellen-JOIN',
        markdown: 'Erstelle eine Abfrage mit **ArtikelNr**, **Artikelname**, **Lieferantenfirma** und **Kategorienname**.\n\nJOIN über `Artikel`, `Lieferant` und `Kategorie`.',
        seed: SEED_NORDWIND, check: { type: 'result', expectedSql: 'SELECT a.ArtikelNr, a.Artikelname, l.Firma, k.Kategoriename FROM Artikel a JOIN Lieferant l ON a.LieferantenNr = l.LieferantenNr JOIN Kategorie k ON a.KategorieNr = k.KategorieNr ORDER BY a.ArtikelNr' },
        sql: 'SELECT a.ArtikelNr, a.Artikelname, l.Firma, k.Kategoriename FROM Artikel a JOIN Lieferant l ON a.LieferantenNr = l.LieferantenNr JOIN Kategorie k ON a.KategorieNr = k.KategorieNr ORDER BY a.ArtikelNr;', hint: 'Zwei JOINs: Artikel → Lieferant und Artikel → Kategorie',
        checklist: ['Zwei JOINs', 'Vier Spalten ausgegeben'],
      },
      {
        id: 'nw-j-06', title: 'Artikelliste für Bestellung 11031',
        markdown: 'Gib die **Artikelliste** für die Bestellung **11031** mit **Einzelpreis** und **Gesamtpreis** aus, wobei sich der Gesamtpreis aus `Anzahl * Einzelpreis` ergibt.\n\nJOIN zwischen `Bestelldetails` und `Artikel`.',
        seed: SEED_NORDWIND, check: { type: 'result', expectedSql: 'SELECT a.Artikelname, bd.Einzelpreis, ROUND(bd.Anzahl * bd.Einzelpreis, 2) AS gesamtpreis FROM Bestelldetails bd JOIN Artikel a ON bd.ArtikelNr = a.ArtikelNr WHERE bd.BestellNr = 11031 ORDER BY a.ArtikelNr' },
        sql: 'SELECT a.Artikelname, bd.Einzelpreis, ROUND(bd.Anzahl * bd.Einzelpreis, 2) AS gesamtpreis FROM Bestelldetails bd JOIN Artikel a ON bd.ArtikelNr = a.ArtikelNr WHERE bd.BestellNr = 11031 ORDER BY a.ArtikelNr;', hint: 'JOIN Bestelldetails mit Artikel, Berechnung: Anzahl * Einzelpreis',
        checklist: ['JOIN Bestelldetails mit Artikel', 'Gesamtpreis = Anzahl * Einzelpreis', 'WHERE BestellNr = 11031'],
      },
      {
        id: 'nw-j-07', title: 'Anzahl Kunden von Buchanan',
        markdown: 'Für wie viele **verschiedene Kunden** hat der Mitarbeiter **Buchanan** schon Bestellungen abgewickelt?\n\nJOIN zwischen `Bestellung` und `Personal`, verwende `COUNT(DISTINCT ...)`.',
        seed: SEED_NORDWIND, check: { type: 'result', expectedSql: "SELECT COUNT(DISTINCT b.KundenCode) FROM Bestellung b JOIN Personal p ON b.PersonalNr = p.PersonalNr WHERE p.Nachname = 'Buchanan'" },
        sql: "SELECT COUNT(DISTINCT b.KundenCode) AS anzahl FROM Bestellung b JOIN Personal p ON b.PersonalNr = p.PersonalNr WHERE p.Nachname = 'Buchanan';", hint: 'COUNT(DISTINCT b.KundenCode) mit JOIN auf Personal WHERE Nachname',
        checklist: ['JOIN Bestellung mit Personal', 'COUNT DISTINCT KundenCode', 'Ergebnis: 4'],
      },
      {
        id: 'nw-j-08', title: 'Lagerbestand prüfen',
        markdown: "Sind alle Artikel der Bestellung der **Rattlesnake Canyon Grocery** vom **1998-05-06** in ausreichender Anzahl auf Lager?\n\nZeige **Artikelname**, **Lagerbestand**, **bestellte Menge** und eine Spalte `ausreichend` mit `'ja'` oder `'nein'`.",
        seed: SEED_NORDWIND, check: { type: 'result', expectedSql: "SELECT a.Artikelname, a.Lagerbestand, bd.Anzahl, CASE WHEN a.Lagerbestand >= bd.Anzahl THEN 'ja' ELSE 'nein' END AS ausreichend FROM Bestelldetails bd JOIN Artikel a ON bd.ArtikelNr = a.ArtikelNr JOIN Bestellung b ON bd.BestellNr = b.BestellNr WHERE b.KundenCode = 'RATTC' AND b.Bestelldatum = '1998-05-06'" },
        sql: "SELECT a.Artikelname, a.Lagerbestand, bd.Anzahl, CASE WHEN a.Lagerbestand >= bd.Anzahl THEN 'ja' ELSE 'nein' END AS ausreichend FROM Bestelldetails bd JOIN Artikel a ON bd.ArtikelNr = a.ArtikelNr JOIN Bestellung b ON bd.BestellNr = b.BestellNr WHERE b.KundenCode = 'RATTC' AND b.Bestelldatum = '1998-05-06';", hint: 'Drei-Tabellen-JOIN mit CASE WHEN für Lagerprüfung',
        checklist: ['Drei-Tabellen-JOIN', 'CASE WHEN für Bestandsprüfung', 'WHERE KundenCode und Datum'],
      },
      {
        id: 'nw-j-09', title: 'Artikel von Richter Supermarkt',
        markdown: 'Welche **Artikel** hat der Kunde **Richter Supermarkt** schon bestellt?\n\nGib eine **sortierte Liste** der Artikelnamen aus (jeden Artikel nur einmal).',
        seed: SEED_NORDWIND, check: { type: 'result', expectedSql: "SELECT DISTINCT a.Artikelname FROM Bestelldetails bd JOIN Bestellung b ON bd.BestellNr = b.BestellNr JOIN Artikel a ON bd.ArtikelNr = a.ArtikelNr WHERE b.KundenCode = 'RICSU' ORDER BY a.Artikelname" },
        sql: "SELECT DISTINCT a.Artikelname FROM Bestelldetails bd JOIN Bestellung b ON bd.BestellNr = b.BestellNr JOIN Artikel a ON bd.ArtikelNr = a.ArtikelNr WHERE b.KundenCode = 'RICSU' ORDER BY a.Artikelname;", hint: 'JOIN Bestelldetails → Bestellung → Artikel, DISTINCT, ORDER BY',
        checklist: ['Drei-Tabellen-JOIN', 'DISTINCT', 'ORDER BY', 'WHERE KundenCode = RICSU'],
      },
      {
        id: 'nw-j-10', title: 'Kunden von Escargots Nouveaux',
        markdown: 'Welche **Kunden** haben schon Artikel der Firma **Escargots Nouveaux** gekauft?\n\nGib eine sortierte Liste der Kundennamen aus. Dies erfordert einen **Fünf-Tabellen-JOIN**: `Kunde` → `Bestellung` → `Bestelldetails` → `Artikel` → `Lieferant`.',
        seed: SEED_NORDWIND, check: { type: 'result', expectedSql: "SELECT DISTINCT k.Firma FROM Kunde k JOIN Bestellung b ON k.KundenCode = b.KundenCode JOIN Bestelldetails bd ON b.BestellNr = bd.BestellNr JOIN Artikel a ON bd.ArtikelNr = a.ArtikelNr JOIN Lieferant l ON a.LieferantenNr = l.LieferantenNr WHERE l.Firma = 'Escargots Nouveaux' ORDER BY k.Firma" },
        sql: "SELECT DISTINCT k.Firma FROM Kunde k JOIN Bestellung b ON k.KundenCode = b.KundenCode JOIN Bestelldetails bd ON b.BestellNr = bd.BestellNr JOIN Artikel a ON bd.ArtikelNr = a.ArtikelNr JOIN Lieferant l ON a.LieferantenNr = l.LieferantenNr WHERE l.Firma = 'Escargots Nouveaux' ORDER BY k.Firma;", hint: 'Fünf-Tabellen-JOIN: Kunde → Bestellung → Bestelldetails → Artikel → Lieferant',
        checklist: ['Fünf-Tabellen-JOIN', 'DISTINCT', 'ORDER BY'],
      },
    ],
  },
  {
    id: 'nordwind-mixed',
    title: 'Vermischtes zur Nordwind-Datenbank',
    difficulty: 'hard',
    tags: ['Aggregation', 'GROUP BY', 'JOIN', 'CASE', 'Subquery', 'Nordwind'],
    description: '10 gemischte Aufgaben mit der Nordwind-Datenbank. Von einfachen WHERE-Abfragen bis zu komplexen Umsatzberechnungen mit GROUP BY, JOINs, CASE und HAVING.',
    defaultSeed: SEED_NORDWIND,
    tasks: [
      {
        id: 'nw-m-01', title: 'Preis ab 50',
        markdown: 'Zeige **alle Artikel** an, deren **Einzelpreis mindestens 50** beträgt.',
        seed: SEED_NORDWIND, check: { type: 'result', expectedSql: 'SELECT * FROM Artikel WHERE Einzelpreis >= 50' },
        sql: 'SELECT * FROM Artikel WHERE Einzelpreis >= 50;', hint: 'WHERE Einzelpreis >= 50',
        checklist: ['WHERE mit >=', 'Hinweis: Kein Artikel kostet >= 50 in den Beispieldaten'],
      },
      {
        id: 'nw-m-02', title: 'Bestellungen im Jahr 1996',
        markdown: 'Wie viele **Bestellungen** gab es im Jahr **1996**?\n\nNutze einen Datumsvergleich mit `>=` und `<` oder `strftime()`.',
        seed: SEED_NORDWIND, check: { type: 'result', expectedSql: "SELECT COUNT(*) FROM Bestellung WHERE Bestelldatum >= '1996-01-01' AND Bestelldatum < '1997-01-01'" },
        sql: "SELECT COUNT(*) AS anzahl FROM Bestellung WHERE Bestelldatum >= '1996-01-01' AND Bestelldatum < '1997-01-01';", hint: 'WHERE Bestelldatum >= \'1996-01-01\' AND Bestelldatum < \'1997-01-01\'',
        checklist: ['Datumsfilter', 'COUNT(*)', 'Ergebnis: 1'],
      },
      {
        id: 'nw-m-03', title: 'Nachbestellungen',
        markdown: 'Welche **Artikel** müssen **nachbestellt** werden?\n\nEin Artikel muss nachbestellt werden, wenn der `Lagerbestand` kleiner als der `Mindestbestand` ist. Zeige alle Spalten an.',
        seed: SEED_NORDWIND, check: { type: 'result', expectedSql: 'SELECT * FROM Artikel WHERE Lagerbestand < Mindestbestand' },
        sql: 'SELECT * FROM Artikel WHERE Lagerbestand < Mindestbestand;', hint: 'WHERE Lagerbestand < Mindestbestand',
        checklist: ['WHERE mit Spaltenvergleich', "Sir Rodney's Scones und Ipoh Coffee"],
      },
      {
        id: 'nw-m-04', title: 'Gesamtumsatz 1998 mit Rabatt',
        markdown: 'Berechne den **Gesamtumsatz** für das Jahr **1998**. Der **Rabatt** soll berücksichtigt werden.\n\nUmsatz = SUM(Einzelpreis × Anzahl × (1 − Rabatt)). Runde auf 2 Dezimalstellen.',
        seed: SEED_NORDWIND, check: { type: 'result', expectedSql: "SELECT ROUND(SUM(bd.Einzelpreis * bd.Anzahl * (1 - bd.Rabatt)), 2) FROM Bestelldetails bd JOIN Bestellung b ON bd.BestellNr = b.BestellNr WHERE b.Bestelldatum >= '1998-01-01' AND b.Bestelldatum < '1999-01-01'" },
        sql: "SELECT ROUND(SUM(bd.Einzelpreis * bd.Anzahl * (1 - bd.Rabatt)), 2) AS gesamtumsatz FROM Bestelldetails bd JOIN Bestellung b ON bd.BestellNr = b.BestellNr WHERE b.Bestelldatum >= '1998-01-01' AND b.Bestelldatum < '1999-01-01';", hint: 'JOIN Bestelldetails mit Bestellung, SUM(Einzelpreis * Anzahl * (1 - Rabatt))',
        checklist: ['JOIN Bestelldetails mit Bestellung', 'Rabatt berücksichtigt: (1 - Rabatt)', 'ROUND auf 2 Stellen', 'Datumsfilter 1998'],
      },
      {
        id: 'nw-m-05', title: 'Jahresumsätze',
        markdown: "Wie Aufgabe 4, nur soll jetzt eine **Liste mit allen Jahresumsätzen** erstellt werden.\n\nZeige `jahr` und `umsatz` (gerundet auf 2 Stellen). Gruppiere mit `strftime('%Y', Bestelldatum)`.",
        seed: SEED_NORDWIND, check: { type: 'result', expectedSql: "SELECT strftime('%Y', b.Bestelldatum) AS jahr, ROUND(SUM(bd.Einzelpreis * bd.Anzahl * (1 - bd.Rabatt)), 2) AS umsatz FROM Bestelldetails bd JOIN Bestellung b ON bd.BestellNr = b.BestellNr GROUP BY jahr ORDER BY jahr" },
        sql: "SELECT strftime('%Y', b.Bestelldatum) AS jahr, ROUND(SUM(bd.Einzelpreis * bd.Anzahl * (1 - bd.Rabatt)), 2) AS umsatz FROM Bestelldetails bd JOIN Bestellung b ON bd.BestellNr = b.BestellNr GROUP BY jahr ORDER BY jahr;", hint: "GROUP BY strftime('%Y', Bestelldatum) mit SUM()",
        checklist: ['GROUP BY Jahr', 'SUM mit Rabatt', 'ORDER BY Jahr'],
      },
      {
        id: 'nw-m-06', title: 'Kategorien bei Federal Shipping',
        markdown: 'Welche **Produktkategorien** werden von der Versandfirma **Federal Shipping** geordert?\n\nGib eine **sortierte Liste** der Kategorienamen aus (jede Kategorie nur einmal).',
        seed: SEED_NORDWIND, check: { type: 'result', expectedSql: "SELECT DISTINCT k.Kategoriename FROM Bestelldetails bd JOIN Bestellung b ON bd.BestellNr = b.BestellNr JOIN Artikel a ON bd.ArtikelNr = a.ArtikelNr JOIN Kategorie k ON a.KategorieNr = k.KategorieNr JOIN Versandfirma v ON b.FirmenNr = v.FirmenNr WHERE v.Firma = 'Federal Shipping' ORDER BY k.Kategoriename" },
        sql: "SELECT DISTINCT k.Kategoriename FROM Bestelldetails bd JOIN Bestellung b ON bd.BestellNr = b.BestellNr JOIN Artikel a ON bd.ArtikelNr = a.ArtikelNr JOIN Kategorie k ON a.KategorieNr = k.KategorieNr JOIN Versandfirma v ON b.FirmenNr = v.FirmenNr WHERE v.Firma = 'Federal Shipping' ORDER BY k.Kategoriename;", hint: 'Vier-Tabellen-JOIN: Bestelldetails → Bestellung → Artikel → Kategorie → Versandfirma',
        checklist: ['Fünf-Tabellen-JOIN inkl. Versandfirma', 'DISTINCT', 'ORDER BY'],
      },
      {
        id: 'nw-m-07', title: 'Top-Artikel nach Verkaufsmenge',
        markdown: 'Erstelle eine **Liste der Artikel**, die am häufigsten verkauft wurden. Zeige `Artikelname` und `gesamtmenge` (SUM der Anzahl). Sortiere absteigend nach Menge.',
        seed: SEED_NORDWIND, check: { type: 'result', expectedSql: 'SELECT a.Artikelname, SUM(bd.Anzahl) AS gesamtmenge FROM Bestelldetails bd JOIN Artikel a ON bd.ArtikelNr = a.ArtikelNr GROUP BY a.ArtikelNr ORDER BY gesamtmenge DESC' },
        sql: 'SELECT a.Artikelname, SUM(bd.Anzahl) AS gesamtmenge FROM Bestelldetails bd JOIN Artikel a ON bd.ArtikelNr = a.ArtikelNr GROUP BY a.ArtikelNr ORDER BY gesamtmenge DESC;', hint: 'JOIN + GROUP BY + SUM(Anzahl) + ORDER BY DESC',
        checklist: ['JOIN Bestelldetails mit Artikel', 'GROUP BY', 'SUM(Anzahl)', 'ORDER BY DESC'],
      },
      {
        id: 'nw-m-08', title: 'Bestellungen mit Rechnungssumme',
        markdown: 'Erstelle eine **sortierte Liste der Bestellungen** mit **Kundenfirma**, **BestellNr** und **Rechnungssumme**.\n\nZur Rechnungssumme gehören die Artikelkosten (mit Rabatt) **plus** die **Frachtkosten**.\n\nRechnungssumme = SUM(Einzelpreis × Anzahl × (1 − Rabatt)) + Frachtkosten.\n\nRunde auf 2 Stellen und sortiere nach BestellNr.',
        seed: SEED_NORDWIND, check: { type: 'result', expectedSql: 'SELECT k.Firma, b.BestellNr, ROUND(SUM(bd.Einzelpreis * bd.Anzahl * (1 - bd.Rabatt)) + b.Frachtkosten, 2) AS rechnungssumme FROM Bestellung b JOIN Kunde k ON b.KundenCode = k.KundenCode JOIN Bestelldetails bd ON b.BestellNr = bd.BestellNr GROUP BY b.BestellNr ORDER BY b.BestellNr' },
        sql: 'SELECT k.Firma, b.BestellNr, ROUND(SUM(bd.Einzelpreis * bd.Anzahl * (1 - bd.Rabatt)) + b.Frachtkosten, 2) AS rechnungssumme FROM Bestellung b JOIN Kunde k ON b.KundenCode = k.KundenCode JOIN Bestelldetails bd ON b.BestellNr = bd.BestellNr GROUP BY b.BestellNr ORDER BY b.BestellNr;', hint: 'GROUP BY BestellNr, SUM(Artikelkosten) + Frachtkosten',
        checklist: ['Drei-Tabellen-JOIN', 'GROUP BY BestellNr', 'SUM(Artikelkosten mit Rabatt) + Frachtkosten', 'ORDER BY BestellNr'],
      },
      {
        id: 'nw-m-09', title: 'Umsatz pro Kategorie 1998',
        markdown: 'Ermittle für das Jahr **1998** den **Umsatz** in den Kategorien **Getränke**, **Süßwaren** und **Fisch/Meeresfrüchte**.\n\nZeige `Kategoriename` und `umsatz` (gerundet, Rabatt berücksichtigt). Sortiere nach Kategoriename.',
        seed: SEED_NORDWIND, check: { type: 'result', expectedSql: "SELECT k.Kategoriename, ROUND(SUM(bd.Einzelpreis * bd.Anzahl * (1 - bd.Rabatt)), 2) AS umsatz FROM Bestelldetails bd JOIN Bestellung b ON bd.BestellNr = b.BestellNr JOIN Artikel a ON bd.ArtikelNr = a.ArtikelNr JOIN Kategorie k ON a.KategorieNr = k.KategorieNr WHERE b.Bestelldatum >= '1998-01-01' AND b.Bestelldatum < '1999-01-01' AND k.Kategoriename IN ('Getränke', 'Süßwaren', 'Fisch/Meeresfrüchte') GROUP BY k.Kategoriename ORDER BY k.Kategoriename" },
        sql: "SELECT k.Kategoriename, ROUND(SUM(bd.Einzelpreis * bd.Anzahl * (1 - bd.Rabatt)), 2) AS umsatz FROM Bestelldetails bd JOIN Bestellung b ON bd.BestellNr = b.BestellNr JOIN Artikel a ON bd.ArtikelNr = a.ArtikelNr JOIN Kategorie k ON a.KategorieNr = k.KategorieNr WHERE b.Bestelldatum >= '1998-01-01' AND b.Bestelldatum < '1999-01-01' AND k.Kategoriename IN ('Getränke', 'Süßwaren', 'Fisch/Meeresfrüchte') GROUP BY k.Kategoriename ORDER BY k.Kategoriename;", hint: 'Vier-Tabellen-JOIN, WHERE mit Jahresfilter und IN(...), GROUP BY Kategorie',
        checklist: ['Vier-Tabellen-JOIN', 'WHERE Jahresfilter + IN', 'GROUP BY + SUM mit Rabatt', 'ORDER BY'],
      },
      {
        id: 'nw-m-10', title: 'Exotic Liquids Werbeaktion',
        markdown: 'Der Lieferant **Exotic Liquids** möchte eine Werbe-Aktion für sein **Getränke**-Sortiment machen.\n\nErstelle für ihn eine Liste der Kunden, die schon **Getränke** seiner Firma mit einem **Mindestumsatz von 100** bestellt haben.\n\nZeige `Firma` und `umsatz` (gerundet). Sortiere nach Umsatz absteigend.\n\n**Tipp:** Fünf-Tabellen-JOIN mit `HAVING` für den Mindestumsatz.',
        seed: SEED_NORDWIND, check: { type: 'result', expectedSql: "SELECT k.Firma, ROUND(SUM(bd.Einzelpreis * bd.Anzahl * (1 - bd.Rabatt)), 2) AS umsatz FROM Kunde k JOIN Bestellung b ON k.KundenCode = b.KundenCode JOIN Bestelldetails bd ON b.BestellNr = bd.BestellNr JOIN Artikel a ON bd.ArtikelNr = a.ArtikelNr JOIN Lieferant l ON a.LieferantenNr = l.LieferantenNr WHERE l.Firma = 'Exotic Liquids' AND a.KategorieNr = (SELECT KategorieNr FROM Kategorie WHERE Kategoriename = 'Getränke') GROUP BY k.Firma HAVING umsatz >= 100 ORDER BY umsatz DESC" },
        sql: "SELECT k.Firma, ROUND(SUM(bd.Einzelpreis * bd.Anzahl * (1 - bd.Rabatt)), 2) AS umsatz FROM Kunde k JOIN Bestellung b ON k.KundenCode = b.KundenCode JOIN Bestelldetails bd ON b.BestellNr = bd.BestellNr JOIN Artikel a ON bd.ArtikelNr = a.ArtikelNr JOIN Lieferant l ON a.LieferantenNr = l.LieferantenNr WHERE l.Firma = 'Exotic Liquids' AND a.KategorieNr = (SELECT KategorieNr FROM Kategorie WHERE Kategoriename = 'Getränke') GROUP BY k.Firma HAVING umsatz >= 100 ORDER BY umsatz DESC;", hint: 'Fünf-Tabellen-JOIN + Subquery für Kategorie + GROUP BY + HAVING umsatz >= 100',
        checklist: ['Fünf-Tabellen-JOIN', 'Subquery für KategorieNr', 'GROUP BY mit HAVING', 'ORDER BY umsatz DESC'],
      },
    ],
  },
];
